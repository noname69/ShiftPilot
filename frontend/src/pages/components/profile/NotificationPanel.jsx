...
import {
  respondAsTarget,
  managerRespondFromNotification,
} from "../../../api/swap";
...
const NotificationPanel = () => {
  ...
  const isActionable = (n) =>
    n.type === "REQUEST_SUBMITTED" && n.referenceId != null;

  const handleTargetRespond = async (n, accepted) => {
    setLoadingId(n.id);
    try {
      await respondAsTarget({
        swapRequestId: n.referenceId,
        accepted,
        comment: null,
      });
      await markAsRead(n.id);
      await fetchNotifications({page: page});
      toast.success(
  ...
  const handleManagerRespond = async (n, decision) => {
    ...
    try {
      ...
      await fetchNotifications({page: page});
      toast.success(
        decision ? "Swap request approved" : "Swap request rejected",
      );
    } catch {
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1 min-w-0">
                    <p
                      className={`text-[12.5px] truncate ${!n.isRead ? "font-semibold text-ink-800" : "text-ink-600"}`}
                    >
                      {n.title}
                    </p>
